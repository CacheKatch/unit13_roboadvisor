...
## The below function is to validate the data
def validate_data(age, investment_amount, intent_request):
    """
    this is to validate the data provided by the user
    """
    # validate the age is between 0 and 65 years old
    if age is not None:
        age = parse_int(age)
        if age >= 65 or age < 0:
            return build_validation_result(
                False,
                "age",
                " your age should be between 0 and 65 to invest on retirement,"
...
def recommend_portfolio(intent_request):
    ...
    initial_recomend = "100% bonds (AGG), 0% equities (SPY)"
...
def dispatch(intent_request):
    ...
    if intent_name == "RecommendPortfolio":  ## changed the name to match my aws lambda function
